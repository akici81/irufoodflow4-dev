  ...
  return (
    <DashboardLayout title="Malzeme Talebi" subtitle="Yeni haftalık malzeme listesi oluşturun">
      <div className="max-w-7xl animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-8 pb-20">
        ...
        {/* Üst Bilgi Kartı */}
        <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 flex flex-wrap items-center gap-8">
          <div className="flex-1 min-w-[280px]">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Ders Seçimi</label>
            <select value={secilenDers} onChange={(e) => setSecilenDers(e.target.value)}
              className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-red-100 outline-none transition-all">
              <option value="">Lütfen Ders Seçiniz...</option>
              {dersler.map((d) => <option key={d.id} value={d.id}>{d.kod} - {d.ad}</option>)}
            </select>
          </div>
          <div className="w-full md:w-64">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Uygulama Haftası</label>
            <select value={secilenHafta} onChange={(e) => setSecilenHafta(e.target.value)}
              className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-red-100 outline-none">
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="font-black text-slate-800 tracking-tight">Listede Olmayan Ürün Ekle</h2>
              </div>
              <div className="flex flex-wrap gap-4 items-end">
                <div className="flex-[2] min-w-[200px]">
                  <input value={manuelUrun} onChange={(e) => setManuelUrun(e.target.value)} placeholder="Ürün adını yazın..."
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3.5 text-sm focus:ring-2 focus:ring-red-100 outline-none transition-all" />
                </div>
                <div className="flex-1 min-w-[100px]">
                  <input type="number" value={manuelMiktar} onChange={(e) => setManuelMiktar(e.target.value)} placeholder="Miktar"
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3.5 text-sm outline-none" />
                </div>
                <div className="w-32">
                  <select value={manuelOlcu} onChange={(e) => setManuelOlcu(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3.5 text-sm font-bold outline-none">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                <h2 className="font-black text-slate-800 tracking-tight italic">Malzeme Havuzu</h2>
                <div className="relative">
                  <input value={aramaMetni} onChange={(e) => setAramaMetni(e.target.value)} placeholder="Malzeme ara..."
                    className="bg-white border border-slate-200 rounded-full px-6 py-2 text-xs focus:ring-2 focus:ring-[#8B1A1A]/10 outline-none w-48 md:w-64 transition-all" />
                </div>
              </div>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {urunler.filter(u => !aramaMetni || u.urunAdi.toLowerCase().includes(aramaMetni.toLowerCase())).map((u) => {
                      ...
                      return (
                        <tr key={u.id} className={`group transition-all ${secili ? "bg-red-50/50" : "hover:bg-slate-50"}`}>
                          <td className="px-8 py-5 text-center">
                            <input type="checkbox" checked={secili} onChange={(e) => handleCheckbox(u, e.target.checked)}
                              className="w-5 h-5 accent-[#8B1A1A] rounded-lg cursor-pointer" />
                          </td>
                          <td className="px-8 py-5">
                            <p className="font-black text-slate-800 text-sm tracking-tight">{u.urunAdi}</p>
                            <span className="text-[10px] font-bold text-slate-400 uppercase">{u.marka || "Standart"}</span>
                          </td>
                          <td className="px-8 py-5 text-center">
                            <span className="text-xs font-bold text-slate-500 italic">₺{u.fiyat.toFixed(2)} / {u.olcu}</span>
                          </td>
                          <td className="px-8 py-5">
                            {secili ? (
                                ...
                                {bilgi.serbest ? (
                                  <div className="flex items-center bg-white border border-red-200 rounded-xl px-3 py-1 shadow-inner">
                                    <input type="text" inputMode="decimal" placeholder="0,00"
                                      ...
                                      className="w-16 bg-transparent text-right text-xs font-black text-[#8B1A1A] outline-none" />
                                  ...
                                  <div className="flex items-center gap-1 bg-white border border-red-100 rounded-xl p-1 shadow-sm">
                                    ...
                                    <span className="w-12 text-center text-xs font-black text-slate-700">{miktar} <span className="text-[8px] block text-slate-300 leading-none tracking-tighter uppercase">{u.olcu}</span></span>
                                    <button onClick={() => handleArttir(u)} className="w-8 h-8 flex items-center justify-center bg-red-50 hover:bg-[#8B1A1A] hover:text-white text-[#8B1A1A] rounded-lg font-black transition">+</button>
                                  </div>
                                )}
                              </div>
                            ) : (
                              <div className="text-right"><span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest italic">Miktar Bekleniyor</span></div>
                            )}
                          </td>
                        </tr>
                      );
          ...
          <div className="lg:col-span-1">
            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl sticky top-8 border border-slate-800">
              <div className="flex items-center justify-between mb-8 border-b border-slate-800 pb-6">
                ...
                <span className="bg-[#8B1A1A] text-[10px] px-2 py-1 rounded-md font-black">{Object.keys(seciliUrunler).length} Kalem</span>
              </div>

              {Object.keys(seciliUrunler).length === 0 && !manuelUrun ? (
                <div className="text-center py-12">
                  <span className="text-4xl block mb-4 opacity-20">🛒</span>
                  <p className="text-slate-500 text-[11px] font-bold uppercase tracking-widest">Henüz malzeme seçmediniz</p>
                </div>
              ) : (
                    ...
                    return (
                      <div key={id} className="flex justify-between items-start group">
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-slate-100 truncate tracking-tight">{u.urunAdi}</p>
                  ...
                  {manuelUrun && (
                    <div className="flex justify-between items-start border-t border-slate-800 pt-4">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-tighter">{manuelMiktar || 1} {manuelOlcu}</p>
                      </div>
                      <span className="text-[10px] italic text-slate-600">Manuel</span>
                    </div>
                  )}
                </div>
              )}
              ...
              <button onClick={handleGonder}
                className="w-full bg-[#8B1A1A] hover:bg-red-700 text-white font-black py-4 rounded-2xl transition-all shadow-lg shadow-red-900/40 uppercase text-xs tracking-[0.2em]">
