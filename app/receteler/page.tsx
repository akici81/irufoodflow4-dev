      ...
      <div className="max-w-6xl space-y-5">
        ...
        {(veriYukleniyor || ortakYukleniyor) ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {gosterilecek.map(r => (
              <div key={r.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 flex flex-col gap-3 hover:shadow-md hover:-translate-y-1 transition-all duration-300">
                    ...
                    {aktifSekme === "ortak" ? (
                      ...
                      <>
                        <button onClick={() => handleDuzenleAc(r)}
                          className="text-[10px] font-black text-gray-400 hover:text-blue-600 px-2.5 py-1.5 rounded-xl hover:bg-blue-50 transition uppercase tracking-widest">Düzenle</button>
                        <button onClick={() => handleSil(r.id)}
                          className="text-[10px] font-black text-gray-400 hover:text-red-600 px-2.5 py-1.5 rounded-xl hover:bg-red-50 transition uppercase tracking-widest">Sil</button>
                      </>
                    )}
                  </div>
                </div>
                      ...
                      {oneriAcik && urunOneri.length > 0 && (
                        <div className="absolute left-0 top-full mt-1 z-50 bg-white border border-gray-200 rounded-xl shadow-xl w-72 max-h-52 overflow-y-auto">
                          <div className="px-3 py-1.5 border-b border-gray-100">
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Ürün Havuzu</span>
                          </div>
                          {urunOneri.map((u, i) => (
                            <button
                              key={i}
